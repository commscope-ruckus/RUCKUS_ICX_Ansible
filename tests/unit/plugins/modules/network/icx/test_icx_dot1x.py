...
from ansible_collections.commscope.icx.plugins.modules import icx_dot1x
...
class TestICXAclAssignModule(TestICXModule):
    ...
    def setUp(self):
        ...
        self.mock_load_config = patch('ansible_collections.commscope.icx.plugins.modules.icx_dot1x.load_config')
        self.load_config = self.mock_load_config.start()
        self.mock_exec_command = patch('ansible_collections.commscope.icx.plugins.modules.icx_dot1x.exec_command')
    ...
    def test_icx_dot1x_all_options(self):
        ...
        set_module_args(dict(enable=dict(all=True), guest_vlan=dict(vlan_id=12), macauth_override='present',
                             max_reauth_req=dict(count=4), max_req=dict(count=3), port_control=dict(auto='yes', all=True),
                             timeout=dict(supplicant=32)))
        expected_commands = [
            'authentication',
            'dot1x enable all',
            'dot1x guest-vlan 12',
    ...
    def test_icx_dot1x_all_options_remove(self):
        ...
        set_module_args(dict(enable=dict(all=True, state='absent'), guest_vlan=dict(vlan_id=12, state='absent'), macauth_override='absent',
    ...
    def test_icx_dot1x_port_control_ethernet(self):
        ...
        set_module_args(dict(port_control=dict(force_unauthorized=True, ethernet='1/1/15')))
        expected_commands = [
            'authentication',
