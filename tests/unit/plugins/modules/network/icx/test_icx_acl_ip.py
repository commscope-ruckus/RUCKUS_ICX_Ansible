...
from ansible_collections.commscope.icx.plugins.modules.network.icx import icx_acl_ip
...
class TestICXAclIpModule(TestICXModule):
    ...
    def setUp(self):
        ...
        self.mock_load_config = patch('ansible_collections.commscope.icx.plugins.modules.network.icx.icx_acl_ip.load_config')
        self.load_config = self.mock_load_config.start()
        self.mock_exec_command = patch('ansible_collections.commscope.icx.plugins.modules.network.icx.icx_acl_ip.exec_command')
    ...
    def test_icx_acl_ip_extended_all_options_remove(self):
        ...
        set_module_args(dict(acl_type='extended', acl_name='acl1', state='absent',
                             extended_rules=[(dict(seq_num='10', rule_type='permit', ip_protocol_name='ip', source=dict(host='yes', ip_address='1.1.1.1'),
                                                   destination=dict(ip_address='2.2.2.2', mask='0.0.0.0'), precedence='routine', tos='normal',
                                                   dscp_matching='21', dscp_marking='8', priority_matching='6', priority_marking='6',
                                                   internal_priority_marking='3', log='yes', mirror='yes', state='absent')),
                                             (dict(remark=dict(comment_text='This Denies icmp traffic', state='absent'), seq_num='20', rule_type='deny',
                                                   ip_protocol_name='icmp', source=dict(any='yes'), destination=dict(ip_address='1.1.1.1', mask='0.0.0.0'),
                                                   icmp_num='25', precedence='priority', tos='max-reliability', dscp_matching='21', dscp_marking='8',
    ...
    def test_icx_acl_ip_standard_all_options_remove(self):
        ...
        set_module_args(dict(acl_type='standard', acl_name='acl1', state='absent', accounting='disable',
                             standard_rules=[(dict(seq_num='10', rule_type='permit', host='yes', source_ip='2.2.2.2',
    ...
    def test_icx_acl_ip_extended_deny(self):
        ...
        set_module_args(dict(acl_type='extended', acl_name='acl1',
                             extended_rules=[(dict(rule_type='deny', ip_protocol_name='ip', source=dict(any='yes'),
                                             ...
                                             (dict(rule_type='deny', ip_protocol_name='tcp', source=dict(any='yes'),
                                                   destination=dict(any='yes'), source_comparison_operators=dict(operator='eq', port_num='22'),
                                                   precedence='network'))]))
        expected_commands = ['ip access-list extended acl1',
    ...
    def test_icx_acl_ip_extended_permit(self):
        ...
        set_module_args(dict(acl_type='extended', acl_name='acl1',
                             extended_rules=[(dict(rule_type='permit', ip_protocol_name='ip', source=dict(host='yes', ip_address='1.1.1.1'),
                                                   destination=dict(any='yes'), traffic_policy_name='policy1')),
                                             (dict(rule_type='permit', ip_protocol_name='icmp', source=dict(any='yes'),
                                                   destination=dict(ip_address='1.1.1.1', mask='0.0.0.0'), icmp_num='25')),
                                             (dict(rule_type='permit', ip_protocol_name='tcp', source=dict(any='yes'),
                                                   destination=dict(any='yes'), source_comparison_operators=dict(operator='eq', port_num='22'),
                                                   destination_comparison_operators=dict(operator='range', port_name='ftp')))]))
        expected_commands = ['ip access-list extended acl1',
    ...
    def test_icx_invalid_args_acl_ip_extended(self):
        ...
        set_module_args(dict(acl_name='acl1',
                             extended_rules=[(dict(seq_num='aa', rule_type='permit', ip_protocol_name='ip',
                                                   source=dict(host='yes', ip_address='1.1.1.1'), destination=dict(any='yes')))]))
        result = self.execute_module(failed=True)
